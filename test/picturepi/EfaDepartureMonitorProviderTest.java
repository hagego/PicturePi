...
class EfaDepartureMonitorProviderTest {
	...
	void testGetResponseStreamWithInvalidServer() {
		assertThat(provider.getResponseStream("http://my.dummy.server", ""), is(nullValue()));
	}
	
	@Test
	void testGetResponseStreamWithValidData() {
		assertThat(provider.getResponseStream("http://efastatic.vvs.de/OpenVVSDay", "cafe stoll"), is(notNullValue()));
	}
	
	@Test
	void testGetDepartureListWithValidData() {
		InputStream inputStream = provider.getResponseStream("http://efastatic.vvs.de/OpenVVSDay", "cafe stoll");
